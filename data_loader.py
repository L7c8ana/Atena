import os
import scipy.io
import numpy as np
import pandas as pd
import pickle
from scipy import interpolate
# from _io import save_results2, load_results2
# from SlidingGenerator import SlidingGenerator
from intervals import divide_blocks
from scipy import ndimage
import matplotlib.pyplot as plt
import tensorflow as tf
import math


def save_resultsPKL(results, path = None):
    with open(path,'wb') as f:
        pickle.dump(results,f)

""" function to load results """
def load_resultsPKL(path=None):
    with open(path,'rb') as f:
        results = pickle.load(f)
    return results

def MinMaxScaler(array, scale=[0, 1]):
    array_max = array.max()
    array_min = array.min()
    return scale[0] + (((array - array_min) * (scale[1] - scale[0])) / (array_max - array_min))

# """ Loader method """
# def load_data(data_dir = './data/'):

#     if 'WellData.mat' not in data_dir:
#         data_file = os.path.join(data_dir, 'WellData.mat')
#     else:
#         data_file = f'{data_dir}'
#         data_dir = os.path.dirname(data_dir)
#     assert(os.path.isfile(data_file))

#     """ Load data """
#     WellData = scipy.io.loadmat(data_file)
#     DEPTH = WellData['DataBase']['Depth'][0][0][:,0].astype(np.float32)
#     U = WellData['DataBase']['UBIProc'][0][0].astype(np.float32)
#     F = WellData['DataBase']['FMIRaw'][0][0]
#     PERM = WellData['DataBase']['PERM'][0][0].astype(np.float32)
#     PHIE = WellData['DataBase']['PHIE'][0][0].astype(np.float32)

#     """ Merge & apply log scale to F """
#     F = np.hstack(F[0])
#     F = np.log10(F)
#     F[np.isnan(F) | np.isinf(F)] = 0

#     """ Check if corrected K data is there """
#     if os.path.isfile(os.path.join(data_dir, 'KTIM_CBPF_AJUTS_SHF.csv')):
#         PETRO = pd.read_csv(os.path.join(data_dir, 'KTIM_CBPF_AJUTS_SHF.csv'), sep=';')
#         """ Keep only permeability and depth """
#         PETRO = PETRO[['DEPTH', 'P']]
#         """ Replace decimal comma with decimal point """
#         PETRO['DEPTH'] = PETRO['DEPTH'].apply(lambda x: float(x.split()[0].replace(',', '.')))
#         PETRO['K'] = PETRO['P'].apply(lambda x: float(x.split()[0].replace(',', '.')))
#         PETRO = PETRO.drop('P', axis=1)

#         """ Now crop depth between in common interval """
#         d0 = DEPTH
#         d1 = PETRO['DEPTH']
#         dres0 = np.min(np.diff(np.sort(d0)))
#         dres1 = np.min(np.diff(np.sort(d1)))
#         dres = np.minimum(dres0,dres1)
#         dmin = np.maximum(d0.min(),d1.min())
#         dmax = np.minimum(d0.max(),d1.max())

#         """ Create new depth """
#         dref = np.arange(dmin, dmax, dres)

#         """ Now crop new data """
#         PETRO = PETRO[(d1 >= dmin) & (d1 <= dmax)]

#         """ Crop old data too """
#         idxs2keep = (d0 >= dmin) & (d0 <= dmax)
#         U = U[idxs2keep]
#         F = F[idxs2keep]
#         PERM = PERM[idxs2keep]
#         PHIE = PHIE[idxs2keep]
#         d0 = d0[idxs2keep]

#         """ Interpolate new data """
#         fP = interpolate.interp1d(PETRO['DEPTH'], PETRO['K'])
#         K = fP(dref)

#         """ And interpolate old data too """
#         fU = interpolate.interp2d(np.arange(0,U.shape[1]), d0, U)
#         U = fU(np.arange(0,U.shape[1]), dref)
#         fF = interpolate.interp2d(np.arange(0, F.shape[1]), d0, F)
#         F = fF(np.arange(0, F.shape[1]), dref)
#         fPERM = interpolate.interp1d(d0, PERM[:,0])
#         PERM = fPERM(dref)[:,None]
#         fPHIE = interpolate.interp1d(d0, PHIE[:, 0])
#         PHIE = fPHIE(dref)[:, None]

#         """ Replace PERM by new PERM """
#         PERM = K[:,None]


#     """ Add last dimension to image data """
#     U = U[:, :, None]

#     F = F[:, :, None]

#     """ Normalize images """
#     U = (U - np.nanmin(U)) / np.ptp(U)
#     F = (F - np.nanmin(F)) / np.ptp(F)

#     """ Apply log scale to PERM """
#     PERM = np.log10(PERM)
#     PERM[np.isnan(PERM) | np.isinf(PERM)] = np.nanmin(PERM[~(np.isnan(PERM) | np.isinf(PERM))])

#     # UT = np.zeros((U.shape[0],U.shape[1],3))
#     # print(U.shape)
#     # print(UT.shape)
#     # UT[:, :, 0] = U[:,:,0]
#     # UT[:, :, 1] = np.log10(U[:,:,0])
#     # UT[:, :, 2] = ndimage.gaussian_gradient_magnitude(U[:,:,0], sigma=1)
#     #
#     # plt.rcParams["axes.grid"] = False
#     # fig = plt.figure()
#     # ax1 = fig.add_subplot(131)
#     # ax2 = fig.add_subplot(132)
#     # ax3 = fig.add_subplot(133)
#     #
#     # ax1.imshow(UT[0:250,:,0], cmap="gray")
#     # ax2.imshow(UT[0:250, :, 1], cmap="gray")
#     # ax3.imshow(UT[0:250, :, 2], cmap="gray")
#     # fig.savefig("TestChannelsAcousticImg.png")
#     #
#     # U = UT

#     """ Build data """
#     data = {'Acoustic': U.astype(np.float32),
#             'Resistivity': F.astype(np.float32),
#             'PERM': PERM.astype(np.float32),
#             'PHIE': PHIE.astype(np.float32)
#             }
#     print(U.shape)
#     print(F.shape)
#     print(PERM.shape)
#     print(PHIE.shape)


#     """ Normalize PERM/PHIE """
#     coeffs = {xn: [np.nanmin(data[xn]), np.nanmax(data[xn])] for xn in data}
#     data_norm = {yn: (data[yn] - coeffs[yn][0]) / (coeffs[yn][1] - coeffs[yn][0]) for yn in data}

#     return data, data_norm, coeffs



def load_results2(path):
    with open(path, 'rb') as f:
        results = pickle.load(f)

    return results

# def load_dataClass(data_dirIMG=None, data_dirClass=None, data_dirSave=None, MakeChannels=None, dataPred=True):
#     # inputs : CSVs
#     # outputs : Dict with : Array Data acoustic normalize 0-1 float32 (U) and array labels categorical FDISSOL - int32

#     fileImg = data_dirIMG[:-4]
#     fileClass = data_dirClass[:-4]
    
    

#     if os.path.isfile(fileImg + '.npy') and \
#             os.path.isfile(fileClass + '.npy') and \
#             os.path.isfile(fileClass +  '_Depth' + '.npy'):

#         """ Load data """
#         U = np.load(fileImg + '.npy')
#         FDISSOL = np.load(fileClass + '.npy')
#         Depth = np.load(fileClass +  '_Depth' + '.npy')
#         print("Data Shape:")
#         print("U", U.shape)
#         print("Class", FDISSOL.shape)
#         print("Depth" , Depth.shape)
#     else:
#         CatIMG = pd.read_csv(data_dirIMG, delimiter=";")  # np.genfromtxt(data_dirIMG, skip_header=1, delimiter=";")
#         CatClass = pd.read_csv(data_dirClass, delimiter=",",
#                                skiprows=1)  # np.genfromtxt(data_dirIMG, skip_header=1, delimiter=";")
#         CatClass["m"] = CatClass["m"].apply(lambda x: float(x.split()[0].replace(',', '.')))
#         CatIMG["MD"] = CatIMG["MD"].apply(lambda x: float(x.split()[0].replace(',', '.')))

#         for i in range(180):
#             CatIMG["UBI_AMP[" + str(i) + "]"] = CatIMG["UBI_AMP[" + str(i) + "]"].apply(
#                 lambda x: float(x.split()[0].replace(',', '.')))

#         CatIMGA = np.array(CatIMG.values)
#         U = CatIMGA[1:, 1:-1]
#         CatClassA = np.array(CatClass.values)
#         FDISSOL = CatClassA[1:, 1]
#         Depth = CatClassA[1:, 0]

#         del CatClassA
#         del CatIMGA
#         np.save(fileImg + '.npy', U)
#         np.save(fileClass + '.npy', FDISSOL)
#         np.save(fileClass +  '_Depth' + '.npy', Depth)
#         print("Data Shape:")
#         print("U", U.shape)
#         print("Class", FDISSOL.shape)
#         print("Depth", Depth.shape)

#     HISTBorehole = True
#     if HISTBorehole:
#         ##########
#         # hist poco w1 e w2
#         ImG1 = np.load(os.path.join(data_dirSave, "W1_5315-5738m_AMP_RAW.npy"))
#         ImG2 = np.load(os.path.join(data_dirSave, "W2_5485_5915m_AMP_RAW.npy"))
#         plt.figure()
#         # plt.hist(ImG1)
#         ch_maxC = np.percentile(np.ravel(ImG1), 99.9)
#         ch_minC = -(np.percentile(-np.ravel(ImG1), 99.9))
#         plt.hist(np.ravel(ImG1), bins=np.linspace(ch_minC, ch_maxC, 100), color="green", label="W1")
#         plt.savefig("HistW1.png")
#         plt.figure()
#         ch_maxC = np.percentile(np.ravel(ImG2), 99.9)
#         ch_minC = -(np.percentile(-np.ravel(ImG2), 99.9))
#         plt.hist(np.ravel(ImG2), bins=np.linspace(ch_minC, ch_maxC, 100), color="blue", label="W2")
#         plt.savefig("HistW2.png")



#     # .apply(lambda x: float(x.split()[0].replace(',', '.')))
#     # print(CatIMG)
#     PlotsVert = False
#     if PlotsVert:
#         for u in np.arange(0,U.shape[0],1000):
#             inic = u
#             fin = u + 1000
#             fig = plt.figure()
#             ax1 = fig.add_subplot(121)
#             ax2 = fig.add_subplot(122)
#             # ax1.rcParams["axes.grid"] = False
#             ax1.imshow(U[inic:fin, :], cmap="gray")
#             # ax1.set_axis_off
#             ax1.axis('off')
#             ax2.plot(FDISSOL[inic:fin], Depth[inic:fin], 'go--')
#             ax2.xaxis.tick_top()

#             ax2.set_ylabel('depth[m]')
#             # ax2.set_ylim(50, 0)
#             ax2.set_xlim([0,2])
#             ax2.set_xticks([0,1,2])
#             ax2.set_xlabel('Classes')
#             fig.savefig("ShowData_" + str(inic) + "-" + str(fin) + ".png")

#     """ Define float type """

#     U = np.float32(U)
#     FDISSOL = np.uint8(FDISSOL)


#     if dataPred:
#         a = 0
#         UTrain = np.load(os.path.join(data_dirSave, "W2_5485_5915m_AMP_RAW.npy"))
#         print(UTrain.shape)
#         print(UTrain.max())
#         print(UTrain.min())
#         # print(np.percentile(UTrain, 100.0))
#         # print(-(np.percentile(-UTrain, 100.0)))

#         if not os.path.exists(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy')):
#             print("Not Exist file: " + os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             #FDISSOLTrain = np.load(os.path.join(data_dirSave, "W1_5315-5738m_Facies_Olinto.npy"))
#             ch_max = np.percentile(UTrain, 99.9)
#             print(ch_max)
#             ch_min = -(np.percentile(-UTrain, 99.9))
#             print(ch_min)
#             np.save(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'), np.array([UTrain.min(), UTrain.max()]))
#             CH = np.load(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             U = np.clip(U, CH[0], CH[1])
#         else:
#             CH = np.load(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             print("Exist file: " + os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             ch_max = CH[1]
#             ch_min = CH[0]
#             U = np.clip(U, ch_min, ch_max)
#             print(ch_max)
#             print(ch_min)


#     # if dataPred:
#     #     UTrain = np.load(os.path.join(data_dirSave, "W2_5485_5915m_AMP_RAW.npy"))
#     #     UO = (UTrain - np.nanmin(UTrain)) / np.ptp(UTrain)
#     #
#     #     print(UTrain.shape)
#     #     countsTrain, bin_edgesTrain = np.histogram(UTrain, bins=255)
#     #     counts, bin_edges = np.histogram(UO, bins=255)
#     #     # countsN, bin_edgesN = np.histogram(U, bins=bin_edges, range=(0.0,1.0))
#     #     print(bin_edgesTrain)
#     #     print(len(bin_edgesTrain))
#     #     print(bin_edges)
#     #     print(len(bin_edges))
#     #     kkkk




#     if MakeChannels:
#         ic = -100
#         UO = U.copy()
#         UOV = np.ravel(UO)
#         MIN = np.min([i for i in UOV if (i > ic and not math.isnan(i))])
#         MAX = np.max([i for i in UOV if (i > ic and not math.isnan(i))])
#         UON = (UO - MIN) / (MIN + MAX)
#         print(MAX, MIN)

#         Ulog = np.log10(U)
#         UlogV = np.ravel(Ulog)
#         MIN = np.min([i for i in UlogV if (i > ic and not math.isnan(i))])
#         MAX = np.max([i for i in UlogV if (i > ic and not math.isnan(i))])
#         UlogN = (Ulog - MIN) / (MIN + MAX)
#         print(MAX, MIN)

#         UG = ndimage.gaussian_gradient_magnitude(U, sigma=1)
#         UGV = np.ravel(UG)
#         MIN = np.min([i for i in UGV if (i > ic and not math.isnan(i))])
#         MAX = np.max([i for i in UGV if (i > ic and not math.isnan(i))])
#         UGN = (UG - MIN) / (MIN + MAX)
#         print(MAX, MIN)

#         UT = np.zeros((UO.shape[0], UO.shape[1], 3))
#         UT[:, :, 0] = UO
#         UT[:, :, 1] = Ulog
#         UT[:, :, 2] = UG
#         UTN = np.zeros((UO.shape[0], UO.shape[1],3))
#         UTN[:,:,0] = UON
#         UTN[:, :, 1] = UlogN
#         UTN[:, :, 2] = UGN
#         U = UTN.copy()

#         # plt.rcParams["axes.grid"] = False
#         # fig = plt.figure()
#         # ax1 = fig.add_subplot(131)
#         # ax2 = fig.add_subplot(132)
#         # ax3 = fig.add_subplot(133)
#         #
#         # ax1.imshow(U[0:250, :, 0], cmap="gray")
#         # ax2.imshow(U[0:250, :, 1], cmap="gray")
#         # ax3.imshow(U[0:250, :, 2], cmap="gray")
#         # fig.savefig("Test3ChannelsAcousticImg.png")
#     else:
#         """ Normalize images """
#         UT = U
#         MIN = np.min([i for i in np.ravel(U) if i > 0])
#         MAX = np.max(U)
#         U = (U - MIN) / (MIN + MAX)
#         U = U[:, :, None]
#         # plt.rcParams["axes.grid"] = False
#         # fig = plt.figure()
#         # ax1 = fig.add_subplot(131)
#         # ax1.imshow(U[0:250, :, 0], cmap="gray")
#         # fig.savefig("Test1ChannelAcousticImg.png")


#     # from keras.utils import np_utils

#     # print("MIN and MAX Classes:", FDISSOL.min(), FDISSOL.max())
#     # plt.figure()
#     # plt.hist(FDISSOL)
#     # plt.savefig("HistClass.png")
#     #
#     # plt.figure()
#     # plt.bar(FDISSOL,Depth)
#     # plt.savefig("CurvaClass.png")

#     # FDISSOL = np_utils.to_categorical(FDISSOL.reshape((FDISSOL.shape[0],1)), num_classes=int(FDISSOL.max()+1))


#     # U = np.concatenate((U,U,U),axis=2)
#     F = []
#     for i in FDISSOL:
#         F.append(str(i))

#     FDISSOL = F

#     print(U.shape)
#     print(len(FDISSOL))



#     """ Build data """
#     data = {'Acoustic': UT,
#                  'FDISSOL': FDISSOL,
#                  'Depth': Depth
#                  }

#     data_norm = {'Acoustic': U,
#             'FDISSOL': FDISSOL,
#             'Depth': Depth
#             }

#     """ Normalize  """
#     # coeffs = None #{xn: [np.nanmin(data[xn]), np.nanmax(data[xn])] for xn in data}
#     # data_norm = None #{yn: (data[yn] - coeffs[yn][0]) / (coeffs[yn][1] - coeffs[yn][0]) for yn in data}

#     return data_norm, data

# def load_dataClassNewMarcio(data_dirIMG=None, data_dirClass=None, data_dirSave=None, MakeChannels=None, dataPred=True, DataTrain=None, sep=None):
#     # inputs : CSVs
#     # outputs : Dict with : Array Data acoustic normalize 0-1 float32 (U) and array labels categorical FDISSOL - int32
    
#     print(data_dirSave)
#     fileImg = data_dirIMG[:-4]
#     fileClass = data_dirClass[:-4]

# #     print("############################################################################################")
# #     print("#### LOAD BOREHOLE: " + fileImg)
# #     print("############################################################################################")
    
   
    
#     if os.path.isfile(os.path.join(data_dirSave, "data.pkl")) and os.path.isfile(os.path.join(data_dirSave,"dataNorm.pkl")):
#         print("Load files Dataframe in " + os.path.join(data_dirSave, "data.pkl"))
#         data_norm = load_resultsPKL(path = os.path.join(data_dirSave,"dataNorm.pkl"))
#         data = load_resultsPKL(path = os.path.join(data_dirSave,"data.pkl"))
# #         data_norm = pd.DataFrame(data_norm)
# #         data = pd.DataFrame(data)
        
        
# #     else:
# #         if os.path.isfile(fileImg + '.npy') and \
# #                 os.path.isfile(fileClass + '.npy') and \
# #                 os.path.isfile(fileClass +  '_Depth' + '.npy'):
# #             print("Exist : " + fileImg + '.npy')
# #             print("Exist : " + fileClass + '.npy')
# #             print("Exist : " + fileClass +  '_Depth' + '.npy')

# #             """ Load data """
# #             U = np.load(fileImg + '.npy')
# #             FDISSOL = np.load(fileClass + '.npy')
# #             Depth = np.load(fileClass +  '_Depth' + '.npy')
# #             print("Data Shape:")
# #             print("U", U.shape)
# #             print("Class", FDISSOL.shape)
# #             print("Depth" , Depth.shape)
# #         else:
# #             ##############
# #             ##### condição pra verificar se precisa interpo;=lacao
# #             ##############
# #             CatIMG = pd.read_csv(data_dirIMG, delimiter=";")  # np.genfromtxt(data_dirIMG, skip_header=1, delimiter=";")
# #             CatClass = pd.read_csv(data_dirClass, delimiter=sep)#,
# #                                    # skiprows=1)  # np.genfromtxt(data_dirIMG, skip_header=1, delimiter=";")
# #     #         print(sep)
# #     #         print(CatClass)
# #             CatIMGA = np.array(CatIMG.values)
# #             U = CatIMGA[1:, 1:-1]

# #             if U.shape[1] > 180:
# #                 U = CatIMGA[1:, 1:1+180]

# #             if U.shape[1] < 180:
# #                 U = CatIMGA[1:, 1:]

# #             U = np.float64(U)
# #             ic = np.float64(-100.00)
# #             U = np.where(U <= ic, np.max(U), U)
# #             # U = np.nan_to_num(U, nan=np.max(U))

# #             CatClassA = np.array(CatClass.values)
# #     #         print(CatClassA)
# #     #         print(CatClassA.shape)

# #             DepthAMP = CatIMGA[1:, 0]

# #     #         print(sep)
# #     #         print(CatClassA.shape)


# #             FDISSOL = CatClassA[1:, 1]
# #             Depth = CatClassA[1:, 0]

# #             del CatClassA

# #             print("Data Shape BEFORE interpolation:")
# #             print("U", U.shape)
# #             print("Depth AMP", DepthAMP.shape)
# #             print("Class", FDISSOL.shape)
# #             print("Depth Dissol", Depth.shape)

# #             ####### INTERPOLATION
# #             PETRO = {"Depth": Depth, "FDISSOL": FDISSOL}
# #             """ Now crop depth between in common interval """
# #             d0 = CatIMGA[1:, 0] # depth data big
# #             d1 = PETRO["Depth"]
# #             for v in d0:
# #                 if isinstance(v, str):
# #                     print(type(v))
# #                     print(v)
# #                     d0 = np.float64(CatIMGA[1:, 0]) # depth data big
# #                     d1 = np.float64(PETRO["Depth"])
# #                     break

# #             # d2 = FDISSOL
# #             dres0 = np.min(np.diff(np.sort(d0)))
# #             dres1 = np.min(np.diff(np.sort(d1)))

# #             dres = np.minimum(dres0, dres1)
# #             dmin = np.maximum(d0.min(), d1.min())
# #             dmax = np.minimum(d0.max(), d1.max())

# #             """ Create new depth """
# #             dref = np.arange(dmin, dmax, dres)

# #             """ Now crop new data """
# #             hh = []
# #             for j in d1:
# #                 if j >= dmin or j <= dmax:
# #                     hh.append(j)

# #             PETRO["Depth"] = hh
# #             # print(PETRO["Depth"])



# #             """ Crop old data too """
# #             idxs2keep = (d0 >= dmin) & (d0 <= dmax)
# #             # U = U[idxs2keep]
# #             # F = F[idxs2keep]
# #             # PERM = PERM[idxs2keep]
# #             # PHIE = PHIE[idxs2keep]
# #             d0 = d0[idxs2keep]
# #             U = U[idxs2keep, :]
# #             DepthAMP = d0

# #             """ Interpolate new data """
# #             fP = interpolate.interp1d(PETRO['Depth'], PETRO["FDISSOL"])
# #             FD = fP(dref)
# #             # print(len(FD))
# #             # print(np.uint8(FD[0:20]))
# #             # print(len(PETRO["FDISSOL"]))
# #             # print(np.uint8(PETRO["FDISSOL"][0:20]))
# #             FDISSOL = FD


# #             # plt.figure()
# #             # plt.plot(Depth, np.arange(0, len(Depth), 1), color="red", label="Depth Orig")
# #             # plt.plot(DepthAMP, np.arange(0, len(DepthAMP), 1), color="blue", label="Depth Interp")
# #             # plt.xlabel("Depth " +  fileClass[-15:-12])
# #             # plt.legend()
# #             # plt.savefig("ComparativoDepthInterp" + fileClass[-15:-12] + ".png")
# #             #
# #             # plt.figure()
# #             # plt.scatter(np.log2(Depth), np.log2(PETRO["FDISSOL"]), color="red", marker='.', alpha=0.3, label="Before Interp")
# #             # plt.scatter(np.log2(DepthAMP), np.log2(FDISSOL), color="blue", marker='.', alpha=0.3, label="After Interp")
# #             # plt.ylabel("FDissol " +  fileClass[-15:-12])
# #             # plt.ylabel("Depth")
# #             # plt.legend()
# #             # plt.savefig("ComparativoDISSOLInterp" + fileClass[-15:-12] + ".png")


# #             if U.shape[0] != FDISSOL.shape[0]:
# #                 print("Error")
# #                 assert U.shape[0] != FDISSOL.shape[0], "Error Invalid Operation shape Image no match shape classifications"

# #             if U.shape[1] != 180:
# #                 assert U.shape[1] != 180, "Error Invalid Operation shape Image no match shape 180 columns"

# #             plt.close()

# #             HISTBorehole = False
# #             if HISTBorehole:
# #                 ##########
# #                 # hist poco w1 e w2
# #                 ImG1 = np.load(os.path.join(data_dirSave, "W1_5315-5738m_AMP_RAW.npy"))
# #                 ImG2 = np.load(os.path.join(data_dirSave, "W2_5485_5915m_AMP_RAW.npy"))
# #                 plt.figure()
# #                 # plt.hist(ImG1)
# #                 ch_maxC = np.percentile(np.ravel(ImG1), 99.9)
# #                 ch_minC = -(np.percentile(-np.ravel(ImG1), 99.9))
# #                 plt.hist(np.ravel(ImG1), bins=np.linspace(ch_minC, ch_maxC, 100), color="green", label="W1")
# #                 # plt.savefig("HistW1.png")
# #                 plt.figure()
# #                 ch_maxC = np.percentile(np.ravel(ImG2), 99.9)
# #                 ch_minC = -(np.percentile(-np.ravel(ImG2), 99.9))
# #                 plt.hist(np.ravel(ImG2), bins=np.linspace(ch_minC, ch_maxC, 100), color="blue", label="W2")
# #                 # plt.savefig("HistW2.png")



# #             # .apply(lambda x: float(x.split()[0].replace(',', '.')))
# #             # print(CatIMG)
# #             PlotsVert = False
# #             if PlotsVert:
# #                 for u in np.arange(0,U.shape[0],1000):
# #                     inic = u
# #                     fin = u + 1000
# #                     fig = plt.figure()
# #                     ax1 = fig.add_subplot(121)
# #                     ax2 = fig.add_subplot(122)
# #                     # ax1.rcParams["axes.grid"] = False
# #                     ax1.imshow(U[inic:fin, :], cmap="gray")
# #                     # ax1.set_axis_off
# #                     ax1.axis('off')
# #                     ax2.plot(FDISSOL[inic:fin], Depth[inic:fin], 'go--')
# #                     ax2.xaxis.tick_top()

# #                     ax2.set_ylabel('depth[m]')
# #                     # ax2.set_ylim(50, 0)
# #                     ax2.set_xlim([0,2])
# #                     ax2.set_xticks([0,1,2])
# #                     ax2.set_xlabel('Classes')
# #                     fig.savefig("ShowData_" + str(inic) + "-" + str(fin) + ".png")

# #             """ Define float type """

# #             U = np.float32(U)
# #             FDISSOL = np.uint8(FDISSOL)


# #             if dataPred:
# #                 a = 0
# #                 # data_dirSave2 = "/home/dados4T/DeepDissolutionClass/dataJC"
# #                 if len(DataTrain) > 1:
# #                     VMax = []
# #                     VMin = []
# #                     for h in DataTrain:
# #                         UTrain = np.load(h)
# #                         # UTrain = np.load(os.path.join(data_dirSave2,h))

# #                         print(UTrain.shape)
# #                         print(UTrain.max())
# #                         print(UTrain.min())

# #                         # CH = np.load(os.path.join(data_dirSave2, 'MinMax' + "_W2_" + '.npy'))
# #                         # print("Exist file: " + os.path.join(data_dirSave2, 'MinMax' + "_W2_" + '.npy'))
# #                         ch_max = np.percentile(UTrain, 100.0)
# #                         ch_min = -(np.percentile(-UTrain, 100.0))
# #                         if not ch_max in VMax:
# #                             VMax.append(ch_max)
# #                         if not ch_min in VMin:
# #                             VMin.append(ch_min)
# #                     ch_maxF = np.array(ch_max).max()
# #                     ch_minF = np.array(ch_min).min()
# #                     U = np.clip(U, ch_minF, ch_maxF)
# #                     print(ch_maxF)
# #                     print(ch_minF)


# #             # if dataPred:
# #             #     UTrain = np.load(os.path.join(data_dirSave, "W2_5485_5915m_AMP_RAW.npy"))
# #             #     UO = (UTrain - np.nanmin(UTrain)) / np.ptp(UTrain)
# #             #
# #             #     print(UTrain.shape)
# #             #     countsTrain, bin_edgesTrain = np.histogram(UTrain, bins=255)
# #             #     counts, bin_edges = np.histogram(UO, bins=255)
# #             #     # countsN, bin_edgesN = np.histogram(U, bins=bin_edges, range=(0.0,1.0))
# #             #     print(bin_edgesTrain)
# #             #     print(len(bin_edgesTrain))
# #             #     print(bin_edges)
# #             #     print(len(bin_edges))
# #             #     kkkk




# #             if MakeChannels:
# #                 # ic = -100
# #                 # U = np.where(U <= ic, np.max(U), U)
# #                 # U = np.nan_to_num(U, nan=np.max(U))
# #                 # print(U.max(), U.min())

# #                 UO = U.copy()
# #                 UOV = np.ravel(UO)
# #                 MIN = np.min([i for i in UOV if (i > ic and not math.isnan(i))])
# #                 MAX = np.max([i for i in UOV if (i > ic and not math.isnan(i))])

# #                 UON = (UO - MIN) / (MIN + MAX)
# #                 print(MAX, MIN)

# #                 Ulog = np.log10(U)
# #                 UlogV = np.ravel(Ulog)
# #                 MIN = np.min([i for i in UlogV if (i > ic and not math.isnan(i))])
# #                 MAX = np.max([i for i in UlogV if (i > ic and not math.isnan(i))])

# #                 UlogN = (Ulog - MIN) / (MIN + MAX)
# #                 print(MAX, MIN)

# #                 UG = ndimage.gaussian_gradient_magnitude(U, sigma=1)
# #                 UGV = np.ravel(UG)
# #                 MIN = np.min([i for i in UGV if (i > ic and not math.isnan(i))])
# #                 MAX = np.max([i for i in UGV if (i > ic and not math.isnan(i))])

# #                 UGN = (UG - MIN) / (MIN + MAX)
# #                 print(MAX, MIN)

# #                 UT = np.zeros((UO.shape[0], UO.shape[1], 3))
# #                 UT[:, :, 0] = np.nan_to_num(UO)
# #                 UT[:, :, 1] = np.nan_to_num(Ulog)
# #                 UT[:, :, 2] = np.nan_to_num(UG)
# #                 # UT = np.where(UT<=-900, 1, UT)

# #                 print(UT.max(), UT.min())


# #                 UTN = np.zeros((UO.shape[0], UO.shape[1], 3))
# #                 UTN[:, :, 0] = UON
# #                 UTN[:, :, 1] = UlogN
# #                 UTN[:, :, 2] = UGN
# #                 print(UTN.max(), UTN.min())
# #                 U = UTN.copy()
# #             else:
# #                 """ Normalize images """
# #                 UT = U
# #                 MIN = np.min([i for i in np.ravel(U) if (i > ic and not math.isnan(i))])
# #                 MAX = np.max([i for i in np.ravel(U) if (i > ic and not math.isnan(i))])
# #                 U = (U - MIN) / (MIN + MAX)
# #                 U = U[:, :, None]
# #                 # plt.rcParams["axes.grid"] = False
# #                 # fig = plt.figure()
# #                 # ax1 = fig.add_subplot(131)
# #                 # ax1.imshow(U[0:250, :, 0], cmap="gray")
# #                 # fig.savefig("Test1ChannelAcousticImg.png")


# #             # from keras.utils import np_utils

# #             # print("MIN and MAX Classes:", FDISSOL.min(), FDISSOL.max())
# #             # plt.figure()
# #             # plt.hist(FDISSOL)
# #             # plt.savefig("HistClass.png")
# #             #
# #             # plt.figure()
# #             # plt.bar(FDISSOL,Depth)
# #             # plt.savefig("CurvaClass.png")

# #             # FDISSOL = np_utils.to_categorical(FDISSOL.reshape((FDISSOL.shape[0],1)), num_classes=int(FDISSOL.max()+1))


# #             # U = np.concatenate((U,U,U),axis=2)
# #             F = []
# #             for i in FDISSOL:
# #                 F.append(str(i))

# #             FDISSOL = F

# #             print(U.shape)
# #             print(len(FDISSOL))

# #             print("Data Shape AFTER interpolation:")
# #             print("U", U.shape)
# #             print("U max no norm", UT.max())
# #             print("U max no norm", UT.min())
# #             print("Depth AMP", DepthAMP.shape)
# #             print("Class", len(FDISSOL))
# #             print("Depth Dissol", Depth.shape)

# #             """ Build data """
# #             data = {'Acoustic': UT,
# #                          'FDISSOL': FDISSOL,
# #                          'Depth': DepthAMP
# #                          }

# #             data_norm = {'Acoustic': U,
# #                     'FDISSOL': FDISSOL,
# #                     'Depth': DepthAMP
# #                     }

# #             save_resultsPKL(data_norm, path = os.path.join(data_dirSave, "dataNorm.pkl"))
# #             save_resultsPKL(data, path = os.path.join(data_dirSave, "data.pkl"))
# #     else:
# #         """ Normalize  """
#         # coeffs = None #{xn: [np.nanmin(data[xn]), np.nanmax(data[xn])] for xn in data}
#         # data_norm = None #{yn: (data[yn] - coeffs[yn][0]) / (coeffs[yn][1] - coeffs[yn][0]) for yn in data}
# #         data_norm = None
# #         data = None
# #         data_norm = load_resultsPKL(path = os.path.join(data_dirSave,"dataNorm.pkl"))
# #         data = load_resultsPKL(path = os.path.join(data_dirSave,"data.pkl"))
# #         print(data_norm)

#     return data_norm, data

def load_dataClassNewMarcio(data_dirIMG=None, data_dirClass=None, data_dirSave=None, MakeChannels=None, dataPred=True, DataTrain=None, sep=None, LabelWell="W0", Blind=None):
    # inputs : CSVs
    # outputs : Dict with : Array Data acoustic normalize 0-1 float32 (U) and array labels categorical FDISSOL - int32
    
    print("Folder actual: " + data_dirSave)
    fileImg = data_dirIMG[:-4]
    fileClass = data_dirClass[:-4]
    

#     print("############################################################################################")
#     print("#### LOAD BOREHOLE: " + fileImg)
#     print("############################################################################################")
    
    filePKL = os.path.join(data_dirSave, "data" + LabelWell + ".pkl")
    filePKLNorm = os.path.join(data_dirSave, "dataNorm" + LabelWell + ".pkl")
    if os.path.isfile(filePKL) and os.path.isfile(filePKLNorm):
        print("Load files Dataframe in " + filePKLNorm)
        data_norm = load_resultsPKL(path = filePKLNorm)
        data = load_resultsPKL(path = filePKL)
#         data_norm = pd.DataFrame(data_norm)
#         data = pd.DataFrame(data)
        
        
    else:
        
        if os.path.isfile(fileImg + '.npy') and \
                os.path.isfile(fileClass + '.npy') and \
                os.path.isfile(fileClass +  '_Depth' + '.npy'):
            print("Exist : " + fileImg + '.npy')
            print("Exist : " + fileClass + '.npy')
            print("Exist : " + fileClass +  '_Depth' + '.npy')

            """ Load data """
            U = np.load(fileImg + '.npy')
            FDISSOL = np.load(fileClass + '.npy')
            Depth = np.load(fileClass +  '_Depth' + '.npy')
            print("Data Shape:")
            print("U", U.shape)
            print("Class", FDISSOL.shape)
            print("Depth" , Depth.shape)
            """ Build data """
            data = {'Acoustic': UT,
                         'FDISSOL': FDISSOL,
                         'Depth': DepthAMP
                         }

            data_norm = {'Acoustic': U,
                    'FDISSOL': FDISSOL,
                    'Depth': DepthAMP
                    }

            save_resultsPKL(data_norm, path = os.path.join(data_dirSave, "dataNorm.pkl"))
            save_resultsPKL(data, path = os.path.join(data_dirSave, "data.pkl"))
        else:
            ##############
            ##### condição pra verificar se precisa interpo;=lacao
            ##############
            CatIMGv = pd.read_csv(data_dirIMG, delimiter=",") 
            CatIMGpv = pd.read_csv(data_dirIMG, delimiter=";")
            if CatIMGv.shape[1] > 1:
                CatIMG = CatIMGv
            else:
                CatIMG = CatIMGpv
            del CatIMGv
            del CatIMGpv
            print("Loading data Images in: " + data_dirIMG)
#                 print(data_dirIMG[i])
#                 print(CatIMG)
            CatIMGA = np.array(CatIMG.values)
            if not Blind:
                CatClassv = pd.read_csv(data_dirClass, delimiter=",") 
    #                 print(CatClassv.shape)
                CatClasspv = pd.read_csv(data_dirClass, delimiter=";") 
    #                 print(CatClasspv.shape)
                if CatClassv.shape[1] > 1:
                    CatClass = CatClassv
                else:
                    CatClass = CatClasspv
                del CatClassv
                del CatClasspv

                print("Loading data Classes in: " + data_dirClass)
    #                 print(data_dirClass[i])
    #                 print(CatClass)

            else:
                CatClass = np.zeros((CatIMGA.shape[0],2))
            U = CatIMGA[1:, 1:-1]

            if U.shape[1] > 180:
                U = CatIMGA[1:, 1:1+180]

            if U.shape[1] < 180:
                U = CatIMGA[1:, 1:]

            U = np.float64(U)
            ic = np.float64(-100.00)
#             U = np.where(U <= ic, np.max(U), U)

            CatClassA = np.array(CatClass.values)

            DepthAMP = CatIMGA[1:, 0]


            FDISSOL = CatClassA[1:, 1]
            Depth = CatClassA[1:, 0]

            del CatClassA

            print("Data Shape BEFORE interpolation:")
            print("U", U.shape)
            print("Depth AMP", DepthAMP.shape)
            print("Class", FDISSOL.shape)
            print("Depth Dissol", Depth.shape)

            ####### INTERPOLATION
            PETRO = {"Depth": Depth, "FDISSOL": FDISSOL}
            """ Now crop depth between in common interval """
            d0 = CatIMGA[1:, 0] # depth data big
            d1 = PETRO["Depth"]
            for v in d0:
                if isinstance(v, str):
#                     print(type(v))
#                     print(v)
                    d0 = np.float64(CatIMGA[1:, 0]) # depth data big
                    d1 = np.float64(PETRO["Depth"])
                    break
            if not Blind:
                # d2 = FDISSOL
                dres0 = np.min(np.diff(np.sort(d0)))
                dres1 = np.min(np.diff(np.sort(d1)))

                dres = np.minimum(dres0, dres1)
                dmin = np.maximum(d0.min(), d1.min())
                dmax = np.minimum(d0.max(), d1.max())

                """ Create new depth """
                dref = np.arange(dmin, dmax, dres)

                """ Now crop new data """
                hh = []
                for j in d1:
                    if j >= dmin or j <= dmax:
                        hh.append(j)

                PETRO["Depth"] = hh


                """ Crop old data too """
                idxs2keep = (d0 >= dmin) & (d0 <= dmax)
                d0 = d0[idxs2keep]
                U = U[idxs2keep, :]
                DepthAMP = d0

                """ Interpolate new data """
                fP = interpolate.interp1d(PETRO['Depth'], PETRO["FDISSOL"])
                FD = fP(dref)
                FDISSOL = FD

            if U.shape[0] != FDISSOL.shape[0]:
                print("Error")
                assert U.shape[0] != FDISSOL.shape[0], "Error Invalid Operation shape Image no match shape classifications"

            if U.shape[1] != 180:
                assert U.shape[1] != 180, "Error Invalid Operation shape Image no match shape 180 columns"

          
            """ Define float type """

            U = np.float32(U)
            FDISSOL = np.uint8(FDISSOL)


            if MakeChannels:
       
                UO = U.copy()
                UOV = np.ravel(UO)
                MIN = np.min([i for i in UOV if (i > ic and not math.isnan(i))])
                MAX = np.max([i for i in UOV if (i > ic and not math.isnan(i))])

                UON = (UO - MIN) / (MIN + MAX)
#                 print(MAX, MIN)

                Ulog = np.log10(U)
                UlogV = np.ravel(Ulog)
                MIN = np.min([i for i in UlogV if (i > ic and not math.isnan(i))])
                MAX = np.max([i for i in UlogV if (i > ic and not math.isnan(i))])

                UlogN = (Ulog - MIN) / (MIN + MAX)
                print("Max and Min log images")
                print(MAX, MIN)

                UG = ndimage.gaussian_gradient_magnitude(U, sigma=1)
                UGV = np.ravel(UG)
                MIN = np.min([i for i in UGV if (i > ic and not math.isnan(i))])
                MAX = np.max([i for i in UGV if (i > ic and not math.isnan(i))])

                UGN = (UG - MIN) / (MIN + MAX)
                print("Max and Min gauss images")
                
                print(MAX, MIN)

                UT = np.zeros((UO.shape[0], UO.shape[1], 3))
                UT[:, :, 0] = np.nan_to_num(UO)
                UT[:, :, 1] = np.nan_to_num(Ulog)
                UT[:, :, 2] = np.nan_to_num(UG)
                # UT = np.where(UT<=-900, 1, UT)

                print(UT.max(), UT.min())


                UTN = np.zeros((UO.shape[0], UO.shape[1], 3))
                UTN[:, :, 0] = UON
                UTN[:, :, 1] = UlogN
                UTN[:, :, 2] = UGN
                print(UTN.max(), UTN.min())
                U = UTN.copy()
            else:
                """ Normalize images """
                UT = U
                MIN = np.min([i for i in np.ravel(U) if (i > ic and not math.isnan(i))])
                MAX = np.max([i for i in np.ravel(U) if (i > ic and not math.isnan(i))])
                U = (U - MIN) / (MIN + MAX)
                U = U[:, :, None]

            F = []
            for i in FDISSOL:
                F.append(str(i))

            FDISSOL = F

#             print(U.shape)
#             print(len(FDISSOL))

            print("Data Shape AFTER interpolation:")
            print("U", U.shape)
            print("U max no norm", UT.max())
            print("U max no norm", UT.min())
            print("U max norm", U.max())
            print("U max norm", U.min())
            print("Depth AMP", DepthAMP.shape)
            print("Class", len(FDISSOL))
            print("Depth Dissol", Depth.shape)

            """ Build data """
            data = {'Acoustic': UT,
                         'FDISSOL': FDISSOL,
                         'Depth': DepthAMP
                         }

            data_norm = {'Acoustic': U,
                    'FDISSOL': FDISSOL,
                    'Depth': DepthAMP
                    }
            
            """ Save data """
#             U = np.save(fileImg + '.npy', U)
#             FDISSOL = np.save(fileClass + '.npy', FDISSOL)
#             Depth = np.save(fileClass +  '_Depth' + '.npy', DepthAMP)

            save_resultsPKL(data_norm, path = filePKLNorm)
            save_resultsPKL(data, path = filePKL)
#     else:
#         """ Normalize  """
        # coeffs = None #{xn: [np.nanmin(data[xn]), np.nanmax(data[xn])] for xn in data}
        # data_norm = None #{yn: (data[yn] - coeffs[yn][0]) / (coeffs[yn][1] - coeffs[yn][0]) for yn in data}
#         data_norm = None
#         data = None
#         data_norm = load_resultsPKL(path = os.path.join(data_dirSave,"dataNorm.pkl"))
#         data = load_resultsPKL(path = os.path.join(data_dirSave,"data.pkl"))
#         print(data_norm)

    return data_norm, data

def load_dataClassNewMarcioTrain(data_dirIMG=None, data_dirClass=None, data_dirSave=None, MakeChannels=None, dataPred=True, DataTrain=None, LabelWell=None):
    # inputs : CSVs
    # outputs : Dict with : Array Data acoustic normalize 0-1 float32 (U) and array labels categorical FDISSOL - int32
#     U_ALL = np.array([])
    FDISSOL_ALL = []
    DepthAMP_ALL = []
    LabelWell = "_".join(LabelWell)
    print("Folder actual: " + data_dirSave)
#     fileImg = data_dirIMG[:-4]
#     fileClass = data_dirClass[:-4]
    

#     print("############################################################################################")
#     print("#### LOAD BOREHOLE: " + fileImg)
#     print("############################################################################################")
    
#     filePKL = os.path.join(data_dirSave, "data" + LabelWell + ".pkl")
    filePKLNorm = os.path.join(data_dirSave, "dataNorm" + LabelWell + ".pkl")
    
    if os.path.isfile(filePKLNorm):#
        print("Load files Dataframe in " + filePKLNorm)
        data_norm = load_resultsPKL(path = filePKLNorm)
#         data = load_resultsPKL(path = filePKL)
#         data_norm = pd.DataFrame(data_norm)
#         data = pd.DataFrame(data)
        
        
    else:
        for i, j in enumerate(data_dirIMG):
            fileImg = data_dirIMG[i][:-4]
            fileClass = data_dirClass[i][:-4]
#             print(data_dirIMG[i])
           
            if os.path.isfile(fileImg + '.npy') and \
                    os.path.isfile(fileClass + '.npy') and \
                    os.path.isfile(fileClass +  '_Depth' + '.npy'):
                print("Exist : " + fileImg + '.npy')
                print("Exist : " + fileClass + '.npy')
                print("Exist : " + fileClass +  '_Depth' + '.npy')

                """ Load data """
                U = np.load(fileImg + '.npy')
                FDISSOL = np.load(fileClass + '.npy')
                DepthAMP = np.load(fileClass +  '_Depth' + '.npy')
                print("Data Shape:")
                print("U", U.shape)
                print("Class", FDISSOL.shape)
                print("Depth" , DepthAMP.shape)
                U_ALL = np.concatenate([U_ALL, U])
                FDISSOL_ALL.append(FDISSOL)
                DepthAMP_ALL.append(DepthAMP)
#                 """ Build data """
#                 data = {'Acoustic': UT,
#                              'FDISSOL': FDISSOL,
#                              'Depth': DepthAMP
#                              }

#                 data_norm = {'Acoustic': U,
#                         'FDISSOL': FDISSOL,
#                         'Depth': DepthAMP
#                         }

#                 save_resultsPKL(data_norm, path = os.path.join(data_dirSave, "dataNorm.pkl"))
#                 save_resultsPKL(data, path = os.path.join(data_dirSave, "data.pkl"))
            else:
                ##############
                ##### condição pra verificar se precisa interpo;=lacao
                ##############
                try:
                    CatIMG = pd.read_csv(data_dirIMG[i], delimiter=";") 
                except:
                    CatIMG = pd.read_csv(data_dirIMG[i], delimiter=",")
                print("Loading data Images in: " + data_dirIMG[i])
#                 print(data_dirIMG[i])
#                 print(CatIMG)
                try:
                    CatClass = pd.read_csv(data_dirClass[i], delimiter=";")
                except:
                    CatClass = pd.read_csv(data_dirClass[i], delimiter=",")
#                 print(CatClassv.shape)
                
                print("Loading data Classes in: " + data_dirClass[i])
#                 print(data_dirClass[i])
#                 print(CatClass)
                
                
                CatIMGA = np.array(CatIMG.values)
#                 print(CatIMGA.shape)
                U = CatIMGA[1:, 1:-1]
#                 print(U.shape)
                
                if U.shape[1] > 180:
                    U = CatIMGA[1:, 1:1+180]

                if U.shape[1] < 180:
                    U = CatIMGA[1:, 1:]

                U = np.float64(U)
#                 print(U.max(), U.min())
                
                ic = np.float64(-100.00)
#                 U = np.where(U <= ic, np.min(U), U)
#                 print(U)
                
                CatClassA = np.array(CatClass.values)

                DepthAMP = CatIMGA[1:, 0]


                FDISSOL = CatClassA[1:, 1]
                Depth = CatClassA[1:, 0]

                del CatClassA

                print("Data Shape BEFORE interpolation:")
                print("U", U.shape)
                print("Depth AMP", DepthAMP.shape)
                print("Class", FDISSOL.shape)
                print("Depth Dissol", Depth.shape)

                ####### INTERPOLATION
                print("Making Interpolation ...")
                PETRO = {"Depth": Depth, "FDISSOL": FDISSOL}
                """ Now crop depth between in common interval """
                d0 = CatIMGA[1:, 0] # depth data big
                d1 = PETRO["Depth"]
                for v in d0:
                    if isinstance(v, str):
    #                     print(type(v))
    #                     print(v)
                        d0 = np.float64(CatIMGA[1:, 0]) # depth data big
                        d1 = np.float64(PETRO["Depth"])
                        break

                # d2 = FDISSOL
                dres0 = np.min(np.diff(np.sort(d0)))
                dres1 = np.min(np.diff(np.sort(d1)))

                dres = np.minimum(dres0, dres1)
                dmin = np.maximum(d0.min(), d1.min())
                dmax = np.minimum(d0.max(), d1.max())

                """ Create new depth """
                dref = np.arange(dmin, dmax, dres)

                """ Now crop new data """
                hh = []
                for j in d1:
                    if j >= dmin or j <= dmax:
                        hh.append(j)

                PETRO["Depth"] = hh


                """ Crop old data too """
                idxs2keep = (d0 >= dmin) & (d0 <= dmax)
                d0 = d0[idxs2keep]
                U = U[idxs2keep, :]
                DepthAMP = d0

                """ Interpolate new data """
                fP = interpolate.interp1d(PETRO['Depth'], PETRO["FDISSOL"])
                FD = fP(dref)
                FDISSOL = FD

                if U.shape[0] != FDISSOL.shape[0]:
                    print("Error")
                    assert U.shape[0] != FDISSOL.shape[0], "Error Invalid Operation shape Image no match shape classifications"

                if U.shape[1] != 180:
                    assert U.shape[1] != 180, "Error Invalid Operation shape Image no match shape 180 columns"


                """ Define float type """

                U = np.float32(U)
                FDISSOL = np.uint8(FDISSOL)


#                 if MakeChannels:
#                     print("Making 3 channels ....")
#                     UO = U.copy()
#                     UO = np.nan_to_num(UO, nan=0.0)
#                     UOV = np.ravel(UO)
#                     MIN = np.min([k for k in UOV if (k > ic and not math.isnan(k))])
#                     MAX = np.max([k for k in UOV if (k > ic and not math.isnan(k))])

#                     UON = (UO - MIN) / (MIN + MAX)
#                     print("Max and Min original images normalize:")
#                     print(UON.max(), UON.min())

#                     Ulog = np.log10(U)
#                     Ulog = np.nan_to_num(Ulog, nan=0.0)
#                     UlogV = np.ravel(Ulog)
#                     MIN = np.min([k for k in UlogV if (k > ic and not math.isnan(k))])
#                     MAX = np.max([k for k in UlogV if (k > ic and not math.isnan(k))])

#                     UlogN = (Ulog - MIN) / (MIN + MAX)
#                     print("Max and Min log images normalize:")
#                     print(UlogN.max(), UlogN.min())

#                     UG = ndimage.gaussian_gradient_magnitude(U, sigma=1)
#                     UG = np.nan_to_num(UG, nan=0.0)
#                     UGV = np.ravel(UG)
#                     MIN = np.min([k for k in UGV if (k > ic and not math.isnan(k))])
#                     MAX = np.max([k for k in UGV if (k > ic and not math.isnan(k))])

#                     UGN = (UG - MIN) / (MIN + MAX)
#                     print("Max and Min gauss images normalize:")

#                     print(UGN.max(), UGN.min())

#                     UT = np.zeros((UO.shape[0], UO.shape[1], 3))
#                     UT[:, :, 0] = np.nan_to_num(UO)
#                     UT[:, :, 1] = np.nan_to_num(Ulog)
#                     UT[:, :, 2] = np.nan_to_num(UG)
#                     # UT = np.where(UT<=-900, 1, UT)
#                     print("Max and Min original images 3 channels after verify nan values:")
#                     print(UT[:, :, 0].max(), UT[:, :, 0].min())
#                     print(UT[:, :, 1].max(), UT[:, :, 1].min())
#                     print(UT[:, :, 2].max(), UT[:, :, 2].min())


#                     UTN = np.zeros((UO.shape[0], UO.shape[1], 3))
#                     UTN[:, :, 0] = UON
#                     UTN[:, :, 1] = UlogN
#                     UTN[:, :, 2] = UGN
# #                     print(UTN.max(), UTN.min())
#                     U = UTN.copy()
#                 else:
#                     """ Normalize images """
#                     UT = U
#                     UT = np.nan_to_num(UT, nan=0.0)
#                     MIN = np.min([k for k in np.ravel(U) if (k > ic and not math.isnan(k))])
#                     MAX = np.max([k for k in np.ravel(U) if (k > ic and not math.isnan(k))])
#                     U = (U - MIN) / (MIN + MAX)
#                     U = U[:, :, None]

                F = []
                for k in FDISSOL:
                    F.append(str(k))

                FDISSOL = F

    #             print(U.shape)
    #             print(len(FDISSOL))

                print("Data Shape AFTER interpolation:")
                print("U", U.shape)
#                 print("U max no norm", UT.max())
#                 print("U max no norm", UT.min())
                print("U max", U.max())
                print("U max", U.min())
                print("Depth AMP", DepthAMP.shape)
                print("Class", len(FDISSOL))
                print("Depth Dissol", Depth.shape)
                print(i)
                if i == 0.0:
                    U_ALL = U.copy()
                    DepthAMP_ALL = DepthAMP.copy()
                    FDISSOL_ALL = np.array(FDISSOL).copy()
                
                print("U_ALL Shape: " + str(U_ALL.shape))
#                 print("U Shape: " + str(U.shape))
                U_ALL = np.concatenate([U_ALL, U])
                FDISSOL_ALL = np.concatenate([FDISSOL_ALL, np.array(FDISSOL)])
                DepthAMP_ALL = np.concatenate([DepthAMP_ALL, np.array(DepthAMP)])

                """ Build data """
#         data = {'Acoustic': UT,
#                      'FDISSOL': FDISSOL,
#                      'Depth': DepthAMP
#                      }
        if MakeChannels:
            print("Making 3 channels ....")
            UO = U_ALL.copy()
            UO = np.nan_to_num(UO, nan=0.0)
#             UOV = np.ravel(UO)
#             MIN = np.min([k for k in UOV if (k > ic and not math.isnan(k))])
#             MAX = np.max([k for k in UOV if (k > ic and not math.isnan(k))])

            UON = MinMaxScaler(UO, scale=[0, 1])#(UO - MIN) / (MIN + MAX)
            print("Max and Min original images normalize:")
            print(UON.max(), UON.min())

            Ulog = np.log10(U_ALL)
            Ulog = np.nan_to_num(Ulog, nan=0.0)
#             UlogV = np.ravel(Ulog)
#             MIN = np.min([k for k in UlogV if (k > ic and not math.isnan(k))])
#             MAX = np.max([k for k in UlogV if (k > ic and not math.isnan(k))])

            UlogN = MinMaxScaler(Ulog, scale=[0, 1])
            print("Max and Min log images normalize:")
            print(UlogN.max(), UlogN.min())

            UG = ndimage.gaussian_gradient_magnitude(U_ALL, sigma=1)
            UG = np.nan_to_num(UG, nan=0.0)
#             UGV = np.ravel(UG)
#             MIN = np.min([k for k in UGV if (k > ic and not math.isnan(k))])
#             MAX = np.max([k for k in UGV if (k > ic and not math.isnan(k))])

            UGN = MinMaxScaler(UG, scale=[0, 1])#(UG - MIN) / (MIN + MAX)
            print("Max and Min gauss images normalize:")

            print(UGN.max(), UGN.min())

            UT = np.zeros((UO.shape[0], UO.shape[1], 3))
            UT[:, :, 0] = np.nan_to_num(UO)
            UT[:, :, 1] = np.nan_to_num(Ulog)
            UT[:, :, 2] = np.nan_to_num(UG)
            # UT = np.where(UT<=-900, 1, UT)
            print("Max and Min original images 3 channels after verify nan values:")
            print(UT[:, :, 0].max(), UT[:, :, 0].min())
            print(UT[:, :, 1].max(), UT[:, :, 1].min())
            print(UT[:, :, 2].max(), UT[:, :, 2].min())


            UTN = np.zeros((UO.shape[0], UO.shape[1], 3))
            UTN[:, :, 0] = UON
            UTN[:, :, 1] = UlogN
            UTN[:, :, 2] = UGN
#                     print(UTN.max(), UTN.min())
            U = UTN.copy()
        else:
            """ Normalize images """
            UT = U_ALL
            UT = np.nan_to_num(UT, nan=0.0)
            MIN = np.min([k for k in np.ravel(U) if (k > ic and not math.isnan(k))])
            MAX = np.max([k for k in np.ravel(U) if (k > ic and not math.isnan(k))])
            UTN = (U - MIN) / (MIN + MAX)
            UTN = UTN[:, :, None]


        print("Data Shape Saved in PKL:")
        print("U_ALL shape", UTN.shape)
        print("U max norm", UTN.max())
        print("U max norm", UTN.min())
        print("Depth AMP shape", np.array(DepthAMP_ALL).shape)
        print("Class shape", np.array(FDISSOL_ALL).shape)
        
        data_norm = {'Acoustic': UTN,
                'FDISSOL': FDISSOL_ALL,
                'Depth': np.array(DepthAMP_ALL)
                }

        """ Save data """
#             U = np.save(fileImg + '.npy', U)
#             FDISSOL = np.save(fileClass + '.npy', FDISSOL)
#             Depth = np.save(fileClass +  '_Depth' + '.npy', DepthAMP)

        save_resultsPKL(data_norm, path = filePKLNorm)
#         save_resultsPKL(data, path = filePKL)
#     else:
#         """ Normalize  """
        # coeffs = None #{xn: [np.nanmin(data[xn]), np.nanmax(data[xn])] for xn in data}
        # data_norm = None #{yn: (data[yn] - coeffs[yn][0]) / (coeffs[yn][1] - coeffs[yn][0]) for yn in data}
#         data_norm = None
#         data = None
#         data_norm = load_resultsPKL(path = os.path.join(data_dirSave,"dataNorm.pkl"))
#         data = load_resultsPKL(path = os.path.join(data_dirSave,"data.pkl"))
#         print(data_norm)

    return data_norm#, data

# def load_dataClass2Borehole(data_dirIMG=None, data_dirClass=None, data_dirSave=None, MakeChannels=None, dataPred=None, DataTrain=None, sep=None):
#     UAll = []
#     FAll = []
#     DAll = []
#     DAll2 = []

#     for nn, tt, xx in zip(range(len(data_dirIMG)), data_dirIMG, data_dirClass):
#         print(tt)
#         print(xx)
#         print(sep)
#         print(nn)
#         data_norm, data = load_dataClassNewMarcio(data_dirIMG=tt, data_dirClass=xx,
#                                                   data_dirSave=data_dirSave,
#                                                   MakeChannels=MakeChannels, dataPred=dataPred, DataTrain=DataTrain,
#                                                   sep=sep[nn])
#         UAll.append(data_norm['Acoustic'])
#         FAll.append(data_norm['FDISSOL'])
#         # DAll.append(Depth)
#         DAll2.append(data_norm['Depth'])

#     U = np.concatenate((UAll[0], UAll[1]), axis=0)
#     FDISSOL = np.concatenate((FAll[0], FAll[1]), axis=0)
#     # Depth = np.concatenate((DAll[0], DAll[1]), axis=0)
#     DepthAMP = np.concatenate((DAll2[0], DAll2[1]), axis=0)

#     print("Data Shape TWO borehole :")
#     print("U", U.shape)
#     print("Depth AMP", DepthAMP.shape)
#     print("Class", FDISSOL.shape)
#     # print("Depth Dissol", Depth.shape)

#     data_norm = {'Acoustic': U,
#                  'FDISSOL': FDISSOL,
#                  'Depth': DepthAMP
#                  }

#     return data_norm

# def load_dataClass2BoreholeOld(data_dirIMG=None, data_dirClass=None, data_dirSave=None, MakeChannels=None, dataPred=True):
#     # inputs : CSVs
#     # outputs : Dict with : Array Data acoustic normalize 0-1 float32 (U) and array labels categorical FDISSOL - int32
#     UAll = []
#     FAll = []
#     DAll = []
#     DAll2 = []



#     for nn, tt, xx in zip(range(len(data_dirIMG)), data_dirIMG, data_dirClass):
#         fileImg = tt[:-4]
#         # fileClass = xx[:-4]
#         print(tt)
#         CatIMG = pd.read_csv(tt, delimiter=";")  # np.genfromtxt(data_dirIMG, skip_header=1, delimiter=";")
#         CatClass = pd.read_csv(xx, delimiter=",",
#                                skiprows=1)  # np.genfromtxt(data_dirIMG, skip_header=1, delimiter=";")
#         # CatClass["m"] = CatClass["m"].apply(lambda x: float(x.split()[0].replace(',', '.')))
#         # CatIMG["MD"] = CatIMG["MD"].apply(lambda x: float(x.split()[0].replace(',', '.')))

#         # for i in range(180):
#         #     CatIMG["UBI_AMP[" + str(i) + "]"] = CatIMG["UBI_AMP[" + str(i) + "]"].apply(
#         #         lambda x: float(x.split()[0].replace(',', '.')))

#         CatIMGA = np.array(CatIMG.values)
#         U = CatIMGA[1:, 1:-1]
#         if U.shape[1] < 180:
#             U = CatIMGA[1:, 1:]
#         CatClassA = np.array(CatClass.values)
#         FDISSOL = CatClassA[1:, 1]
#         Depth = CatClassA[1:, 0]
#         DepthAMP = CatIMGA[1:, 0]


#         np.save(fileImg + '.npy', U)

#         del CatClassA
#         del CatIMGA
#         del CatClass
#         del CatIMG
#         UAll.append(U)
#         FAll.append(FDISSOL)
#         DAll.append(Depth)
#         DAll2.append(DepthAMP)


#     U = np.concatenate((UAll[0],UAll[1]),axis=0)
#     FDISSOL = np.concatenate((FAll[0],FAll[1]),axis=0)
#     Depth = np.concatenate((DAll[0],DAll[1]),axis=0)
#     DepthAMP = np.concatenate((DAll2[0], DAll2[1]), axis=0)
#     # np.save("U" + 'W1W2.npy', U)
#     # np.save("FDISSOL" + 'W1W2.npy', FDISSOL)
#     # np.save("Depth" + 'W1W2.npy', Depth)
#     print("Data Shape:")
#     print("U", U.shape)
#     print("Class", FDISSOL.shape)
#     print("Depth", Depth.shape)



#     ##########
#     # hist poco w1 e w2
#     # ImG1 = os.path.join(data_dirSave, "W1_5315-5738m_AMP_RAW.csv")
#     plt.figure()
#     plt.hist(U)
#     plt.savefig("DataAmplitudes.png")

#     # .apply(lambda x: float(x.split()[0].replace(',', '.')))
#     # print(CatIMG)
#     PlotsVert = False
#     if PlotsVert:
#         for u in np.arange(0,U.shape[0],1000):
#             inic = u
#             fin = u + 1000
#             fig = plt.figure()
#             ax1 = fig.add_subplot(121)
#             ax2 = fig.add_subplot(122)
#             # ax1.rcParams["axes.grid"] = False
#             ax1.imshow(U[inic:fin, :], cmap="gray")
#             # ax1.set_axis_off
#             ax1.axis('off')
#             ax2.plot(FDISSOL[inic:fin], Depth[inic:fin], 'go--')
#             ax2.xaxis.tick_top()

#             ax2.set_ylabel('depth[m]')
#             # ax2.set_ylim(50, 0)
#             ax2.set_xlim([0,2])
#             ax2.set_xticks([0,1,2])
#             ax2.set_xlabel('Classes')
#             fig.savefig("ShowData_" + str(inic) + "-" + str(fin) + ".png")

#     """ Define float type """

#     U = np.float32(U)
#     FDISSOL = np.uint8(FDISSOL)


#     if dataPred:
#         a = 0
#         UTrain = np.load(os.path.join(data_dirSave, "W2_5485_5915m_AMP_RAW.npy"))
#         print(UTrain.shape)
#         print(UTrain.max())
#         print(UTrain.min())
#         # print(np.percentile(UTrain, 100.0))
#         # print(-(np.percentile(-UTrain, 100.0)))

#         if not os.path.exists(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy')):
#             print("Not Exist file: " + os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             #FDISSOLTrain = np.load(os.path.join(data_dirSave, "W1_5315-5738m_Facies_Olinto.npy"))
#             ch_max = np.percentile(UTrain, 99.9)
#             print(ch_max)
#             ch_min = -(np.percentile(-UTrain, 99.9))
#             print(ch_min)
#             np.save(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'), np.array([UTrain.min(), UTrain.max()]))
#             CH = np.load(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             U = np.clip(U, CH[0], CH[1])
#         else:
#             CH = np.load(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             print("Exist file: " + os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             ch_max = CH[1]
#             ch_min = CH[0]
#             U = np.clip(U, ch_min, ch_max)
#             print(ch_max)
#             print(ch_min)


#     # if dataPred:
#     #     UTrain = np.load(os.path.join(data_dirSave, "W2_5485_5915m_AMP_RAW.npy"))
#     #     UO = (UTrain - np.nanmin(UTrain)) / np.ptp(UTrain)
#     #
#     #     print(UTrain.shape)
#     #     countsTrain, bin_edgesTrain = np.histogram(UTrain, bins=255)
#     #     counts, bin_edges = np.histogram(UO, bins=255)
#     #     # countsN, bin_edgesN = np.histogram(U, bins=bin_edges, range=(0.0,1.0))
#     #     print(bin_edgesTrain)
#     #     print(len(bin_edgesTrain))
#     #     print(bin_edges)
#     #     print(len(bin_edges))
#     #     kkkk




#     if MakeChannels:
#         UO = U.copy()
#         UO = (UO - np.nanmin(UO)) / np.ptp(UO)
#         print(UO.max(), UO.min())
#         Ulog = np.log10(U)
#         Ulog = (Ulog - np.nanmin(Ulog)) / np.ptp(Ulog)
#         print(Ulog.max(), Ulog.min())
#         UG = ndimage.gaussian_gradient_magnitude(U, sigma=1)
#         UG = (UG - np.nanmin(UG)) / np.ptp(UG)
#         print(UG.max(), UG.min())
#         UT = np.zeros((UO.shape[0], UO.shape[1],3))
#         UT[:,:,0] = UO
#         UT[:, :, 1] = Ulog
#         UT[:, :, 2] = UG
#         U = UT.copy()

#         # plt.rcParams["axes.grid"] = False
#         # fig = plt.figure()
#         # ax1 = fig.add_subplot(131)
#         # ax2 = fig.add_subplot(132)
#         # ax3 = fig.add_subplot(133)
#         #
#         # ax1.imshow(U[0:250, :, 0], cmap="gray")
#         # ax2.imshow(U[0:250, :, 1], cmap="gray")
#         # ax3.imshow(U[0:250, :, 2], cmap="gray")
#         # fig.savefig("Test3ChannelsAcousticImg.png")



#     else:
#         """ Normalize images """
#         U = (U - np.nanmin(U)) / np.ptp(U)
#         U = U[:, :, None]
#         plt.rcParams["axes.grid"] = False
#         fig = plt.figure()
#         ax1 = fig.add_subplot(131)
#         ax1.imshow(U[0:250, :, 0], cmap="gray")
#         fig.savefig("Test1ChannelAcousticImg.png")


#     # from keras.utils import np_utils

#     # print("MIN and MAX Classes:", FDISSOL.min(), FDISSOL.max())
#     plt.figure()
#     plt.hist(FDISSOL)
#     plt.savefig("HistClass.png")

#     plt.figure()
#     plt.bar(FDISSOL,Depth)
#     plt.savefig("CurvaClass.png")

#     # FDISSOL = np_utils.to_categorical(FDISSOL.reshape((FDISSOL.shape[0],1)), num_classes=int(FDISSOL.max()+1))


#     # U = np.concatenate((U,U,U),axis=2)
#     F = []
#     for i in FDISSOL:
#         F.append(str(i))

#     FDISSOL = F

#     print(U.shape)
#     print(len(FDISSOL))



#     """ Build data """
#     data_norm = {'Acoustic': U,
#             'FDISSOL': FDISSOL,
#             'Depth': Depth
#             }

#     """ Normalize  """
#     # coeffs = None #{xn: [np.nanmin(data[xn]), np.nanmax(data[xn])] for xn in data}
#     # data_norm = None #{yn: (data[yn] - coeffs[yn][0]) / (coeffs[yn][1] - coeffs[yn][0]) for yn in data}

#     return data_norm

# def load_dataClass2Borehole(data_dirIMG=None, data_dirClass=None, data_dirSave=None, MakeChannels=None, dataPred=None, DataTrain=None, sep=None):
#     UAll = []
#     FAll = []
#     DAll = []
#     DAll2 = []

#     for nn, tt, xx in zip(range(len(data_dirIMG)), data_dirIMG, data_dirClass):
#         print(tt)
#         print(xx)
#         print(sep)
#         print(nn)
#         data_norm, data = load_dataClassNewMarcio(data_dirIMG=tt, data_dirClass=xx,
#                                                   data_dirSave=data_dirSave,
#                                                   MakeChannels=MakeChannels, dataPred=dataPred, DataTrain=DataTrain,
#                                                   sep=sep[nn])
#         UAll.append(data_norm['Acoustic'])
#         FAll.append(data_norm['FDISSOL'])
#         # DAll.append(Depth)
#         DAll2.append(data_norm['Depth'])

#     U = np.concatenate((UAll[0], UAll[1]), axis=0)
#     FDISSOL = np.concatenate((FAll[0], FAll[1]), axis=0)
#     # Depth = np.concatenate((DAll[0], DAll[1]), axis=0)
#     DepthAMP = np.concatenate((DAll2[0], DAll2[1]), axis=0)

#     print("Data Shape TWO borehole :")
#     print("U", U.shape)
#     print("Depth AMP", DepthAMP.shape)
#     print("Class", FDISSOL.shape)
#     # print("Depth Dissol", Depth.shape)

#     data_norm = {'Acoustic': U,
#                  'FDISSOL': FDISSOL,
#                  'Depth': DepthAMP
#                  }

#     return data_norm

# def load_dataClass3Borehole(data_dirIMG=None, data_dirClass=None, data_dirSave=None, MakeChannels=None, dataPred=None, DataTrain=None, sep=None):
#     UAll = []
#     FAll = []
#     DAll = []
#     DAll2 = []

#     for nn, tt, xx in zip(range(len(data_dirIMG)), data_dirIMG, data_dirClass):
#         print(tt)
#         print(xx)
#         print(sep)
#         print(nn)
#         data_norm, data = load_dataClassNewMarcio(data_dirIMG=tt, data_dirClass=xx,
#                                                   data_dirSave=data_dirSave,
#                                                   MakeChannels=MakeChannels, dataPred=dataPred, DataTrain=DataTrain,
#                                                   sep=sep[nn])
#         UAll.append(data_norm['Acoustic'])
#         FAll.append(data_norm['FDISSOL'])
#         # DAll.append(Depth)
#         DAll2.append(data_norm['Depth'])

#     U = np.concatenate((UAll[0], UAll[1], UAll[2]), axis=0)
#     FDISSOL = np.concatenate((FAll[0], FAll[1], FAll[2]), axis=0)
#     # Depth = np.concatenate((DAll[0], DAll[1]), axis=0)
#     DepthAMP = np.concatenate((DAll2[0], DAll2[1], DAll2[2]), axis=0)
#     print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
#     print("Data Shape TWO borehole :")
#     print("U", U.shape)
#     print("Depth AMP", DepthAMP.shape)
#     print("Class", FDISSOL.shape)
#     print("U channel 0:", np.max(U[:,:,0]), np.min(U[:,:,0]))
#     print("U channel 1:", np.max(U[:, :, 1]), np.min(U[:, :, 1]))
#     print("U channel 2:", np.max(U[:, :, 2]), np.min(U[:, :, 2]))
#     # print("Depth Dissol", Depth.shape)

#     data_norm = {'Acoustic': U,
#                  'FDISSOL': FDISSOL,
#                  'Depth': DepthAMP
#                  }

#     return data_norm

# def load_dataClass2BoreholeOld(data_dirIMG=None, data_dirClass=None, data_dirSave=None, MakeChannels=None, dataPred=True):
#     # inputs : CSVs
#     # outputs : Dict with : Array Data acoustic normalize 0-1 float32 (U) and array labels categorical FDISSOL - int32
#     UAll = []
#     FAll = []
#     DAll = []
#     DAll2 = []



#     for nn, tt, xx in zip(range(len(data_dirIMG)), data_dirIMG, data_dirClass):
#         fileImg = tt[:-4]
#         # fileClass = xx[:-4]
#         print(tt)
#         CatIMG = pd.read_csv(tt, delimiter=";")  # np.genfromtxt(data_dirIMG, skip_header=1, delimiter=";")
#         CatClass = pd.read_csv(xx, delimiter=",",
#                                skiprows=1)  # np.genfromtxt(data_dirIMG, skip_header=1, delimiter=";")
#         # CatClass["m"] = CatClass["m"].apply(lambda x: float(x.split()[0].replace(',', '.')))
#         # CatIMG["MD"] = CatIMG["MD"].apply(lambda x: float(x.split()[0].replace(',', '.')))

#         # for i in range(180):
#         #     CatIMG["UBI_AMP[" + str(i) + "]"] = CatIMG["UBI_AMP[" + str(i) + "]"].apply(
#         #         lambda x: float(x.split()[0].replace(',', '.')))

#         CatIMGA = np.array(CatIMG.values)
#         U = CatIMGA[1:, 1:-1]
#         if U.shape[1] < 180:
#             U = CatIMGA[1:, 1:]
#         CatClassA = np.array(CatClass.values)
#         FDISSOL = CatClassA[1:, 1]
#         Depth = CatClassA[1:, 0]
#         DepthAMP = CatIMGA[1:, 0]


#         np.save(fileImg + '.npy', U)

#         del CatClassA
#         del CatIMGA
#         del CatClass
#         del CatIMG
#         UAll.append(U)
#         FAll.append(FDISSOL)
#         DAll.append(Depth)
#         DAll2.append(DepthAMP)


#     U = np.concatenate((UAll[0],UAll[1]),axis=0)
#     FDISSOL = np.concatenate((FAll[0],FAll[1]),axis=0)
#     Depth = np.concatenate((DAll[0],DAll[1]),axis=0)
#     DepthAMP = np.concatenate((DAll2[0], DAll2[1]), axis=0)
#     # np.save("U" + 'W1W2.npy', U)
#     # np.save("FDISSOL" + 'W1W2.npy', FDISSOL)
#     # np.save("Depth" + 'W1W2.npy', Depth)
#     print("Data Shape:")
#     print("U", U.shape)
#     print("Class", FDISSOL.shape)
#     print("Depth", Depth.shape)



#     ##########
#     # hist poco w1 e w2
#     # ImG1 = os.path.join(data_dirSave, "W1_5315-5738m_AMP_RAW.csv")
#     plt.figure()
#     plt.hist(U)
#     plt.savefig("DataAmplitudes.png")

#     # .apply(lambda x: float(x.split()[0].replace(',', '.')))
#     # print(CatIMG)
#     PlotsVert = False
#     if PlotsVert:
#         for u in np.arange(0,U.shape[0],1000):
#             inic = u
#             fin = u + 1000
#             fig = plt.figure()
#             ax1 = fig.add_subplot(121)
#             ax2 = fig.add_subplot(122)
#             # ax1.rcParams["axes.grid"] = False
#             ax1.imshow(U[inic:fin, :], cmap="gray")
#             # ax1.set_axis_off
#             ax1.axis('off')
#             ax2.plot(FDISSOL[inic:fin], Depth[inic:fin], 'go--')
#             ax2.xaxis.tick_top()

#             ax2.set_ylabel('depth[m]')
#             # ax2.set_ylim(50, 0)
#             ax2.set_xlim([0,2])
#             ax2.set_xticks([0,1,2])
#             ax2.set_xlabel('Classes')
#             fig.savefig("ShowData_" + str(inic) + "-" + str(fin) + ".png")

#     """ Define float type """

#     U = np.float32(U)
#     FDISSOL = np.uint8(FDISSOL)


#     if dataPred:
#         a = 0
#         UTrain = np.load(os.path.join(data_dirSave, "W2_5485_5915m_AMP_RAW.npy"))
#         print(UTrain.shape)
#         print(UTrain.max())
#         print(UTrain.min())
#         # print(np.percentile(UTrain, 100.0))
#         # print(-(np.percentile(-UTrain, 100.0)))

#         if not os.path.exists(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy')):
#             print("Not Exist file: " + os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             #FDISSOLTrain = np.load(os.path.join(data_dirSave, "W1_5315-5738m_Facies_Olinto.npy"))
#             ch_max = np.percentile(UTrain, 99.9)
#             print(ch_max)
#             ch_min = -(np.percentile(-UTrain, 99.9))
#             print(ch_min)
#             np.save(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'), np.array([UTrain.min(), UTrain.max()]))
#             CH = np.load(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             U = np.clip(U, CH[0], CH[1])
#         else:
#             CH = np.load(os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             print("Exist file: " + os.path.join(data_dirSave, 'MinMax' + "_W2_" + '.npy'))
#             ch_max = CH[1]
#             ch_min = CH[0]
#             U = np.clip(U, ch_min, ch_max)
#             print(ch_max)
#             print(ch_min)


#     # if dataPred:
#     #     UTrain = np.load(os.path.join(data_dirSave, "W2_5485_5915m_AMP_RAW.npy"))
#     #     UO = (UTrain - np.nanmin(UTrain)) / np.ptp(UTrain)
#     #
#     #     print(UTrain.shape)
#     #     countsTrain, bin_edgesTrain = np.histogram(UTrain, bins=255)
#     #     counts, bin_edges = np.histogram(UO, bins=255)
#     #     # countsN, bin_edgesN = np.histogram(U, bins=bin_edges, range=(0.0,1.0))
#     #     print(bin_edgesTrain)
#     #     print(len(bin_edgesTrain))
#     #     print(bin_edges)
#     #     print(len(bin_edges))
#     #     kkkk




#     if MakeChannels:
#         UO = U.copy()
#         UO = (UO - np.nanmin(UO)) / np.ptp(UO)
#         print(UO.max(), UO.min())
#         Ulog = np.log10(U)
#         Ulog = (Ulog - np.nanmin(Ulog)) / np.ptp(Ulog)
#         print(Ulog.max(), Ulog.min())
#         UG = ndimage.gaussian_gradient_magnitude(U, sigma=1)
#         UG = (UG - np.nanmin(UG)) / np.ptp(UG)
#         print(UG.max(), UG.min())
#         UT = np.zeros((UO.shape[0], UO.shape[1],3))
#         UT[:,:,0] = UO
#         UT[:, :, 1] = Ulog
#         UT[:, :, 2] = UG
#         U = UT.copy()

#         # plt.rcParams["axes.grid"] = False
#         # fig = plt.figure()
#         # ax1 = fig.add_subplot(131)
#         # ax2 = fig.add_subplot(132)
#         # ax3 = fig.add_subplot(133)
#         #
#         # ax1.imshow(U[0:250, :, 0], cmap="gray")
#         # ax2.imshow(U[0:250, :, 1], cmap="gray")
#         # ax3.imshow(U[0:250, :, 2], cmap="gray")
#         # fig.savefig("Test3ChannelsAcousticImg.png")



#     else:
#         """ Normalize images """
#         U = (U - np.nanmin(U)) / np.ptp(U)
#         U = U[:, :, None]
#         plt.rcParams["axes.grid"] = False
#         fig = plt.figure()
#         ax1 = fig.add_subplot(131)
#         ax1.imshow(U[0:250, :, 0], cmap="gray")
#         fig.savefig("Test1ChannelAcousticImg.png")


#     # from keras.utils import np_utils

#     # print("MIN and MAX Classes:", FDISSOL.min(), FDISSOL.max())
#     plt.figure()
#     plt.hist(FDISSOL)
#     plt.savefig("HistClass.png")

#     plt.figure()
#     plt.bar(FDISSOL,Depth)
#     plt.savefig("CurvaClass.png")

#     # FDISSOL = np_utils.to_categorical(FDISSOL.reshape((FDISSOL.shape[0],1)), num_classes=int(FDISSOL.max()+1))


#     # U = np.concatenate((U,U,U),axis=2)
#     F = []
#     for i in FDISSOL:
#         F.append(str(i))

#     FDISSOL = F

#     print(U.shape)
#     print(len(FDISSOL))



#     """ Build data """
#     data_norm = {'Acoustic': U,
#             'FDISSOL': FDISSOL,
#             'Depth': Depth
#             }

#     """ Normalize  """
#     # coeffs = None #{xn: [np.nanmin(data[xn]), np.nanmax(data[xn])] for xn in data}
#     # data_norm = None #{yn: (data[yn] - coeffs[yn][0]) / (coeffs[yn][1] - coeffs[yn][0]) for yn in data}

#     return data_norm


""" Prepare AE data """
def prepare_AE_data(data, batch_size = 32, sliding_window = 21):

    """ Define inputs (also outputs) """
    inputs = ['Acoustic','Resistivity']

    """ Get number of samples """
    nsamples = data[inputs[0]].__len__()

    sliding_function = {yn: 'none' for yn in inputs}

    """ Define indexes (whole thing) """
    delta = (sliding_window - 1) // 2
    indexes = {yn: np.arange(delta, nsamples - delta) for yn in data}

    """ Create Training/Validation Generator """
    generator = SlidingGenerator(x = {xn: data[xn] for xn in inputs},
                                 sliding_window = sliding_window,
                                 sliding_function = sliding_function,
                                 indexes = indexes,
                                 batch_size = batch_size,
                                 shuffle = True)

    val_gen = SlidingGenerator(x = {xn: data[xn] for xn in inputs},
                               sliding_window = sliding_window,
                               sliding_function = sliding_function,
                               indexes = indexes,
                               batch_size = batch_size,
                               shuffle = False)

    """ Calculate data_shape """
    data_shape = {xn: data[xn].shape[1:] for xn in inputs}

    return generator, val_gen, data_shape


""" Prepare AE data """
def prepare_Class_data(data, batch_size = 32, sliding_window = 21):

    """ Define inputs (also outputs) """
    inputs = ['Acoustic']
    outputs = ['FDISSOL']

    """ Get number of samples """
    nsamples = data[inputs[0]].__len__()

    sliding_function = {yn: 'none' for yn in inputs}

    """ Define indexes (whole thing) """
    delta = (sliding_window - 1) // 2
    indexes = {yn: np.arange(delta, nsamples - delta) for yn in data}

    """ Create Training/Validation Generator """
    generator = SlidingGenerator(x = {xn: data[xn] for xn in inputs},
                                 y = {xn: data[xn] for xn in outputs},
                                 sliding_window = sliding_window,
                                 sliding_function = sliding_function,
                                 indexes = indexes,
                                 batch_size = batch_size,
                                 shuffle = True)

    val_gen = SlidingGenerator(x = {xn: data[xn] for xn in inputs},
                               y = {xn: data[xn] for xn in outputs},
                               sliding_window = sliding_window,
                               sliding_function = sliding_function,
                               indexes = indexes,
                               batch_size = batch_size,
                               shuffle = False)

    """ Calculate data_shape """
    data_shape = {xn: data[xn].shape[1:] for xn in inputs}

    return generator, val_gen, data_shape

""" Prepare Regression data """
def prepare_reg_data2(data_norm, codes, smooth_window = 121, sliding_window = None, batch_size = None,
                     validation_split = None, blocksize = None):

    """ Define inputs (also outputs) """
    outputs = ['PERM']#,'PHIE']
    inputs = ["Acoustic"]

    """ Get number of samples """
    delta = (sliding_window - 1) // 2


    """ Crop data """
    X = {'codes': data_norm[yn][delta:-delta] for yn in inputs} #{'codes': codes_cropped}
    Y = {yn: data_norm[yn][delta:-delta] for yn in outputs}

    """ Calculate data_shape """
    input_shape = {xn: X[xn].shape[1:] for xn in X}
    output_shape = {yn: Y[yn].shape[1:] for yn in Y}

    """ Define indexes """
    train_idxs, _, val_idxs = divide_blocks(Z = X['codes'],
                                            blocksize = blocksize,
                                            ratios=((1 - validation_split), 0.0, validation_split),
                                            sliding_window = sliding_window,
                                            silent = False,
                                            discard_overlaps = False)

    delta = (sliding_window - 1) // 2
    train_idxs = train_idxs[np.where((train_idxs >= 2 * delta) & (train_idxs <= len(X['codes']) - 2 * delta))]
    val_idxs = val_idxs[np.where((val_idxs >= 2 * delta) & (val_idxs <= len(X['codes']) - 2 * delta))]

    """ Create Generator """
    train_gen = SlidingGenerator(x = X, y = Y,
                                 sliding_window = sliding_window,
                                 indexes = {ln: train_idxs for ln in outputs + ['codes']},
                                 sliding_function = {**{yn: 'mean' for yn in outputs}, **{'codes': 'none'}},
                                 batch_size = batch_size,
                                 shuffle = True)

    val_gen = SlidingGenerator(x = X, y = Y,
                               sliding_window = sliding_window,
                               indexes = {ln: val_idxs for ln in outputs + ['codes']},
                               sliding_function = {**{yn: 'mean' for yn in outputs}, **{'codes': 'none'}},
                               batch_size = batch_size,
                               shuffle = False)


    full_gen = None


    return train_gen, val_gen, full_gen, input_shape, output_shape, {'train': train_idxs, 'validation': val_idxs}

""" Prepare Regression data """
def prepare_reg_data(data_norm, codes, smooth_window = 121, sliding_window = 21, batch_size = 32,
                     validation_split = .1, blocksize = 21):

    """ Define inputs (also outputs) """
    outputs = ['PERM','PHIE']

    """ Get number of samples """
    delta = (sliding_window - 1) // 2

    """ Crop codes """
    codes = codes[delta:]

    """ Smooth codes """
    codes_cropped = np.concatenate(
        [np.convolve(codes[:, i], np.ones((smooth_window,)) / smooth_window, mode='same')[:, None] for i in
         range(codes.shape[1])], axis=1)

    """ Crop data """
    X = {'codes': codes_cropped}
    Y = {yn: data_norm[yn][delta:-delta] for yn in outputs}

    """ Calculate data_shape """
    input_shape = {xn: X[xn].shape[1:] for xn in X}
    output_shape = {yn: Y[yn].shape[1:] for yn in Y}

    """ Define indexes """
    train_idxs, _, val_idxs = divide_blocks(Z = X['codes'],
                                            blocksize = blocksize,
                                            ratios=((1 - validation_split), 0.0, validation_split),
                                            sliding_window = sliding_window,
                                            silent = False,
                                            discard_overlaps = False)

    delta = (sliding_window - 1) // 2
    train_idxs = train_idxs[np.where((train_idxs >= 2 * delta) & (train_idxs <= len(X['codes']) - 2 * delta))]
    val_idxs = val_idxs[np.where((val_idxs >= 2 * delta) & (val_idxs <= len(X['codes']) - 2 * delta))]

    """ Create Generator """
    train_gen = SlidingGenerator(x = X, y = Y,
                                 sliding_window = sliding_window,
                                 indexes = {ln: train_idxs for ln in outputs + ['codes']},
                                 sliding_function = {**{yn: 'mean' for yn in outputs}, **{'codes': 'none'}},
                                 batch_size = batch_size,
                                 shuffle = True)

    val_gen = SlidingGenerator(x = X, y = Y,
                               sliding_window = sliding_window,
                               indexes = {ln: val_idxs for ln in outputs + ['codes']},
                               sliding_function = {**{yn: 'mean' for yn in outputs}, **{'codes': 'none'}},
                               batch_size = batch_size,
                               shuffle = False)

    full_gen = SlidingGenerator(x=X, y=Y,
                               sliding_window=sliding_window,
                               indexes={ln: np.arange(delta,codes_cropped.__len__()-delta) for ln in outputs + ['codes']},
                               sliding_function={**{yn: 'mean' for yn in outputs}, **{'codes': 'none'}},
                               batch_size=batch_size,
                               shuffle=False)


    return train_gen, val_gen, full_gen, input_shape, output_shape, {'train': train_idxs, 'validation': val_idxs}

""" Prepare Regression data """
def prepare_Class_data2(data_norm, smooth_window = 121, sliding_window = 21, batch_size = 32,
                     validation_split = .1, blocksize = 21):
    print("****************** Prepare Classication Data SlidingGenerator .....")
    """ Define inputs (also outputs) """
    inputs = ['Acoustic']
    outputs = ['FDISSOL']

    """ Get number of samples """
    delta = (sliding_window - 1) // 2


    """ Crop data """
    X = {yn: data_norm[yn][delta:-delta] for yn in inputs}
    Y = {yn: data_norm[yn][delta:-delta] for yn in outputs}
    print(X)

    """ Calculate data_shape """
    input_shape = {xn: X[xn].shape[1:] for xn in X}
    output_shape = {yn: Y[yn].shape[1:] for yn in Y}

    """ Define indexes """
    train_idxs, _, val_idxs = divide_blocks(Z = X['Acoustic'],
                                            blocksize = blocksize,
                                            ratios=((1 - validation_split), 0.0, validation_split),
                                            sliding_window = sliding_window,
                                            silent = False,
                                            discard_overlaps = False)

    delta = (sliding_window - 1) // 2
    train_idxs = train_idxs[np.where((train_idxs >= 2 * delta) & (train_idxs <= len(X['Acoustic']) - 2 * delta))]
    val_idxs = val_idxs[np.where((val_idxs >= 2 * delta) & (val_idxs <= len(X['Acoustic']) - 2 * delta))]

    """ Create Generator """
    train_gen = SlidingGenerator(x = X, y = Y,
                                 sliding_window = sliding_window,
                                 indexes = {ln: train_idxs for ln in data_norm},
                                 sliding_function = {**{yn: 'mean' for yn in outputs}, **{'Acoustic': 'none'}},
                                 batch_size = batch_size,
                                 shuffle = True)

    val_gen = SlidingGenerator(x = X, y = Y,
                               sliding_window = sliding_window,
                               indexes = {ln: val_idxs for ln in data_norm},
                               sliding_function = {**{yn: 'mean' for yn in outputs}, **{'Acoustic': 'none'}},
                               batch_size = batch_size,
                               shuffle = False)




    return train_gen, val_gen, input_shape, output_shape, {'train': train_idxs, 'validation': val_idxs}

